...
class Visitor(object):
    ...
    def reduce_collection(self, result, fs, coll_type):
    ...
    def map(self, value, fs=None, value_type=None):
        ...
        if record_mapped == StopVisiting or isinstance(
            record_mapped, StopVisiting
        ):
    ...
    def map_record(self, record, fs, record_type):
        ...
        if self.apply_records:
            result = self.apply(value, fs, prop, record)
            if result == StopVisiting or isinstance(result, StopVisiting):
        ...
        return self.reduce_record(
            result_dict, fs, record_type,
        )
